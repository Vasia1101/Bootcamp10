...
function resolveDeveloperNodeModule(frb, pkg, name) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const modResolution = yield requireResolveAsync(name, { paths: [frb.cwd] }).catch(NoOp);
        if (!modResolution) {
            return { declared: true, installed: false };
        }
        const modPackageJSON = require(path.join(functionsEmulatorShared_1.findModuleRoot(name, modResolution), "package.json"));
        return {
            declared: true,
            installed: true,
...
function verifyDeveloperNodeModules(frb) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const modBundle of modBundles) {
            const resolution = yield resolveDeveloperNodeModule(frb, pkg, modBundle.name);
...
function InitializeNetworkFiltering(frb) {
    ...
    try {
        ...
        new types_1.EmulatorLog("DEBUG", "runtime-status", `Couldn't find @google-cloud/firestore or google-gax, this may be okay if using @google-cloud/firestore@2.0.0`).log();
    }
    const history = {};
...
function InitializeFirebaseFunctionsStubs(functionsDir) {
    return __awaiter(this, void 0, void 0, function* () {
        const firebaseFunctionsResolution = yield requireResolveAsync("firebase-functions", {
            paths: [functionsDir],
        });
...
function InitializeFirebaseAdminStubs(frb) {
    return __awaiter(this, void 0, void 0, function* () {
        const adminResolution = yield requireResolveAsync("firebase-admin", { paths: [frb.cwd] });
        ...
        const initializeSettings = (userSettings) => {
            ...
            if (!hasInitializedSettings && frb.ports.firestore) {
                app.firestore().settings(Object.assign({ projectId: frb.projectId, port: frb.ports.firestore, servicePath: "localhost", service: "firestore.googleapis.com", sslCreds, customHeaders: {
        ...
        adminModuleProxy = new Proxied(localAdminModule)
            ...
            new types_1.EmulatorLog("SYSTEM", "default-admin-app-used", `config=${config}`).log();
            ...
            app = adminModuleTarget.initializeApp(Object.assign({}, config, opts));
            return app;
        })
        ...
        yield new Promise((resolveEphemeralServer, rejectEphemeralServer) => {
            const handler = (req, res) => __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    new types_1.EmulatorLog("DEBUG", "runtime-status", "Body" + req.rawBody).log();
                    res.on("finish", () => {
                        instance.close();
...
function ProcessBackground(frb, trigger) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        new types_1.EmulatorLog("DEBUG", "runtime-status", `ProcessBackground: proto=${JSON.stringify(proto)}`).log();
        const data = proto.data;
        delete proto.data;
        const context = proto.context ? proto.context : proto;
        if (context.resource && context.resource.name) {
            new types_1.EmulatorLog("DEBUG", "runtime-status", `ProcessBackground: lifting resource.name from resource ${JSON.stringify(context.resource)}`).log();
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield InitializeFirebaseFunctionsStubs(frb.cwd);
