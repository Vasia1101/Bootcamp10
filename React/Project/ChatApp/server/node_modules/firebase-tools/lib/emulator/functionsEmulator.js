...
class FunctionsEmulator {
    ...
    static createHubServer(bundleTemplate, nodeBinary) {
        ...
        const backgroundHandler = (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            const runtime = FunctionsEmulator.startFunctionRuntime(bundleTemplate, triggerId, nodeBinary, proto);
            ...
            yield runtime.ready;
            emulatorLogger_1.EmulatorLogger.log("DEBUG", JSON.stringify(runtime.metadata));
        ...
        const httpsHandler = (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            emulatorLogger_1.EmulatorLogger.log("DEBUG", `[functions] Runtime ready! Sending request! ${JSON.stringify(runtime.metadata)}`);
    ...
    static startFunctionRuntime(bundleTemplate, triggerId, nodeBinary, proto, runtimeOpts) {
    ...
    static handleSystemLog(systemLog) {
        switch (systemLog.type) {
            ...
            case "unidentified-network-access":
                emulatorLogger_1.EmulatorLogger.log("WARN", `Unknown network resource requested!\n   - URL: "${systemLog.data.href}"`);
                break;
...
You can probably fix this by running "npm install ${systemLog.data.name}@latest" in your functions directory.`);
            ...
            case "admin-auto-initialized":
                utils.logBullet("Your code does not appear to initialize the 'firebase-admin' module, so we've done it automatically.\n" +
    ...
    static handleRuntimeLog(log, ignore = []) {
        ...
        switch (log.level) {
            ...
            case "USER":
                ...
                break;
            case "DEBUG":
                emulatorLogger_1.EmulatorLogger.log("DEBUG", log.text);
                break;
            case "INFO":
                emulatorLogger_1.EmulatorLogger.logLabeled("BULLET", "functions", log.text);
                break;
            case "WARN":
                emulatorLogger_1.EmulatorLogger.log("WARN", log.text);
                break;
            case "FATAL":
                emulatorLogger_1.EmulatorLogger.log("WARN", log.text);
                break;
    ...
    start() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.server = FunctionsEmulator.createHubServer(this.bundleTemplate, this.nodeBinary).listen(port, host);
        });
    }
    connect() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const loadTriggers = () => __awaiter(this, void 0, void 0, function* () {
                ...
                for (const definition of toSetup) {
                    if (definition.httpsTrigger) {
                        ...
                        emulatorLogger_1.EmulatorLogger.logLabeled("BULLET", "functions", `HTTP trigger initialized at ${clc.bold(url)}`);
                    }
                    else {
                        ...
                        switch (service) {
                            ...
                            case constants_1.Constants.SERVICE_REALTIME_DATABASE:
                                yield this.addRealtimeDatabaseTrigger(this.projectId, definition);
                                break;
                            default:
                                emulatorLogger_1.EmulatorLogger.log("DEBUG", `Unsupported trigger: ${JSON.stringify(definition)}`);
                                emulatorLogger_1.EmulatorLogger.log("INFO", `Ignoring trigger "${definition.name}" because the service "${service}" is not yet supported.`);
                                break;
                        }
                    }
    ...
    addRealtimeDatabaseTrigger(projectId, definition) {
        ...
        if (!databasePort) {
            emulatorLogger_1.EmulatorLogger.log("INFO", `Ignoring trigger "${definition.name}" because the Realtime Database emulator is not running.`);
        ...
        emulatorLogger_1.EmulatorLogger.logLabeled("BULLET", "functions", `Setting up Realtime Database trigger "${definition.name}"`);
        ...
        return new Promise((resolve, reject) => {
            ...
            request.put(setTriggersPath, {
                ...
                if (res.statusCode === 200) {
                    emulatorLogger_1.EmulatorLogger.logLabeled("SUCCESS", "functions", `Trigger "${definition.name}" has been acknowledged by the Realtime Database emulator.`);
                }
                resolve();
            });
        });
    ...
    addFirestoreTrigger(projectId, definition) {
        ...
        if (!firestorePort) {
            emulatorLogger_1.EmulatorLogger.log("INFO", `Ignoring trigger "${definition.name}" because the Cloud Firestore emulator is not running.`);
            return Promise.resolve();
        }
        const bundle = JSON.stringify({ eventTrigger: definition.eventTrigger });
        emulatorLogger_1.EmulatorLogger.logLabeled("BULLET", "functions", `Setting up Cloud Firestore trigger "${definition.name}"`);
        ...
        return new Promise((resolve, reject) => {
            request.put(`http://localhost:${firestorePort}/emulator/v1/projects/${projectId}/triggers/${definition.name}`, {
                ...
                if (res.statusCode === 200) {
                    emulatorLogger_1.EmulatorLogger.logLabeled("SUCCESS", "functions", `Trigger "${definition.name}" has been acknowledged by the Cloud Firestore emulator.`);
                }
                resolve();
            });
        });
