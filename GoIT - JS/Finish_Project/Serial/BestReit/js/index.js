...
let seialsTopRated = document.querySelector('.seialsTopRated');
let pagesLinkTopRated = document.querySelectorAll('.pagesLink');
...
let getPageSerialsTopRated = function (evt) {
   let event = evt.preventDefault();

   if (evt.currentTarget.dataset.action === 'right') {
      actTopRated = Number(actTopRated) + 1;
      getFilmCardsTopRated(actTopRated);
   } else if (evt.currentTarget.dataset.action === 'left') {
      actTopRated = Number(actTopRated) - 1;
      getFilmCardsTopRated(actTopRated);
   } else if (evt.target.textContent) {
      actTopRated = Number(evt.target.textContent);
      getFilmCardsTopRated(actTopRated);
   }
}
...
let getFilmCardsTopRated = function (i) {
   let apiseialsTopRated = `https://api.themoviedb.org/3/tv/top_rated?api_key=c2b5de19f08adc486af54dcc0c9946be&language=ru-US&page=${i}`;
   seialsTopRated.innerHTML = '';

   fetch(apiseialsTopRated)
...
for (let el of pagesLinkTopRated) {
   el.addEventListener('click', getPageSerialsTopRated);
}
arrowLinkLeft.addEventListener('click', getPageSerialsTopRated);

arrowLinkRight.addEventListener('click', getPageSerialsTopRated);
window.addEventListener('load', getFilmCardsTopRated(actTopRated));
// ======================================NatalyTas======================================//
         ...
         .then(data => {
            ...
            if (data.results.length !== 0) {
              ...
              `;
               ...
               seialsTopRated.innerHTML = `<div class="requestNotFound" height="200px" background-color="blue" ><p>По Вашему запросу ничего не найдено!</p></div>`;
            }
