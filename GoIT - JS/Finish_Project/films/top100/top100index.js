...
let pagesLinkTop = document.querySelectorAll('.pagesLink');
...
let getFilmCardsTop = function(i){




let apiTop =
   `https://api.themoviedb.org/3/movie/top_rated?api_key=c2b5de19f08adc486af54dcc0c9946be&language=ru-RU&page=${i}`;
...
for(let el of pagesLinkTop){
   el.addEventListener('click', getPageTop);
}
arrowLinkLeft.addEventListener('click', getPageTop);

arrowLinkRight.addEventListener('click', getPageTop);
window.addEventListener('load', getFilmCardsTop(actTop));
    ...
    fetch(API)
      ...
      .then(data => {
        ...
        if (data.results.length !== 0) {
          ...
          categoriasTop.innerHTML = `<div class="requestNotFound" height="200px" background-color="blue" ><p>По Вашему запросу ничего не найдено!</p></div>`;
        }
